...
export default function PrincipalConsoleLive() {
  ...
  const updatePrincipalCodeMutation = useMutation({
    mutationFn: async () => (await apiRequest("PATCH", "/api/principal/code", { principalCode: newPrincipalCode })).json(),
    onSuccess: () => {
      setNewPrincipalCode("");
  ...
  const removeCreateClass = (value: string) => {
    setTeacherForm((current) => ({ ...current, classIds: current.classIds.filter((item) => item !== value) }));
  };
    ...
    return (
      <div className="min-h-screen si-gradient">
        <div className="mx-auto flex min-h-screen max-w-md items-center justify-center px-4">
          <motion.div initial="hidden" animate="show" variants={motionIn}>
            <Card className="w-full rounded-[32px] border bg-card/80 p-8 shadow-[var(--shadow-2)] backdrop-blur">
              <div className="flex flex-col items-center gap-6">
                ...
                <div className="text-center">
                  ...
                  <p className="mt-2 text-sm text-foreground/60">Manage staff access, passwords, and class ownership.</p>
                </div>
                <form
  ...
  return (
    <div className="min-h-screen si-gradient">
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-8 md:px-8 md:pt-10">
        <motion.header initial="hidden" animate="show" variants={motionIn} className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <h1 className="mt-4 font-serif text-4xl font-semibold tracking-tight">Principal Console</h1>
            <p className="mt-2 text-sm text-foreground/65">
              Create teacher accounts, control login details, assign classes, and rotate the principal code.
            </p>
          </div>
        ...
        <section className="mt-6 grid gap-4 lg:grid-cols-[0.95fr_1.05fr]">
          <motion.div custom={0.08} initial="hidden" animate="show" variants={motionIn}>
              ...
              <div className="mt-4 grid gap-3">
                {!backendConfigured ? (
                  <div className="rounded-[22px] border border-amber-300/50 bg-amber-50/85 p-4 text-sm text-amber-900">
                    {overviewQuery.data?.backend.detail ?? "Backend setup is still needed before teacher accounts can be created."}
                  </div>
                ) : null}
                ...
                <div className="rounded-[24px] border border-foreground/10 bg-background/45 p-4">
                  ...
                  <p className="mt-1 text-xs text-foreground/58">Classes already assigned to other teachers are hidden here.</p>
                  <div className="mt-4 grid gap-3">
                    <Select value={teacherForm.classPicker} onValueChange={addCreateClass} disabled={!backendConfigured || !availableCreateClasses.length}>
          ...
          <motion.div custom={0.14} initial="hidden" animate="show" variants={motionIn}>
            <Card className="si-card si-noise rounded-[30px] border bg-card/80 p-5 backdrop-blur">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-sm font-semibold">Teacher directory</p>
                  <p className="mt-1 text-xs text-foreground/58">Edit passwords, login IDs, status, and class assignments.</p>
                </div>
                <Badge variant="secondary" className="rounded-full px-3 py-1.5 text-xs">
              ...
              <div className="mt-4 space-y-3">
                {(overviewQuery.data?.teachers ?? []).length ? (
                  overviewQuery.data!.teachers.map((teacher, index) => {
                    const draft = editingTeachers[teacher.id];
                    if (!draft) return null;

                    const availableEditClasses = classOptions.filter(
                      (item) => !takenClassIds.has(item.id) || teacher.classIds.includes(item.id) || draft.classIds.includes(item.id),
                    );
